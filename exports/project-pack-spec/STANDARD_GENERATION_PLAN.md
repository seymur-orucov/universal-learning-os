...
Define the Stage 16.0 plan for future standard Project Pack generation. This document is a planning contract for Stage 16.1; it does not implement generation.
...
- Stage 16.0 MUST NOT implement full standard generation.
- Stage 16.0 MUST NOT overwrite existing standard generated packs automatically.
- Stage 16.1 generation MUST verify exactly 25 output files before reporting success.
- Stage 16.1 generation MUST refuse to write outside `exports/generated/<domain>-standard/`.
- Stage 16.1 generation MUST preserve canonical source traceability in `PACK_MANIFEST.md`.
- Stage 16.1 generation SHOULD support dry-run output before writing.
...
- Templates under `tools/ulos-cli/templates/standard-pack/` are planning templates, not final renderers.
- The CLI does not yet render standard packs.
- Existing standard packs remain committed derived artifacts.
...
Stage 16.1 should extend validation or generator checks to verify the documented 25-file standard output contract and source mapping.
