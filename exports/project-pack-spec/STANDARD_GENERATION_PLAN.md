...
- `COMMAND_CONTINUE_LESSON.md` merges separately labeled `commands/CONTINUE_LESSON.md` and `commands/SAVE_LESSON_TO_NOTION.md`.
...
- `SKILL_LESSON_INSTRUCTOR.md` merges separately labeled `skills/lesson-instructor/SKILL.md` and `skills/notion-lesson-logger/SKILL.md`.
...
- Compact command and skill cores MUST include the Notion command and skill as separately labeled canonical sources.
- Project instructions MUST preserve explicit-only invocation, meaningful-closure placement, connector-confirmed success, Markdown fallback, and evidence/mastery/state separation.
...
- Validation checks file counts, required files, manifest basics, metadata guardrails, Notion journal runtime contracts and privacy boundaries, and launch kit presence, but does not prove full semantic equivalence to canonical sources.
...
- every registry pack includes the optional Notion command and skill, explicit-only and meaningful-boundary rules, fallback draft, confirmed-success rule, mastery/state separation, and no assigned credentials, opaque ids, or active learner-state content;
