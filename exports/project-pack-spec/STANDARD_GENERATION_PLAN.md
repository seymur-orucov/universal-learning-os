...
- `COMMAND_CONTINUE_LESSON.md` merges separately labeled `commands/CONTINUE_LESSON.md`, `commands/SAVE_LESSON.md`, and the `commands/SAVE_LESSON_TO_NOTION.md` compatibility alias.
...
- `SKILL_LESSON_INSTRUCTOR.md` merges separately labeled lesson instructor, shared summary builder, Obsidian exporter, and Notion logger skill sources.
...
- Both profiles MUST use the shared profile source selections in `tools/ulos-cli/src/lib/profile-sources.js`; domain-specific generator branches MUST NOT be required for global commands or skills.
- Compact command and skill cores MUST include the generic export command, Notion alias, summary builder, Obsidian exporter, and Notion logger as separately labeled canonical sources.
- Project instructions MUST preserve explicit-only invocation, generic meaningful-closure placement, default Obsidian and explicit Notion routing, confirmed-success semantics, honest fallbacks, and evidence/mastery/state separation.
...
- Validation checks file counts, required files, manifest basics, metadata guardrails, lesson-note routing, Obsidian and Notion contracts, privacy/state boundaries, and launch kit presence, but does not prove live file creation or full semantic equivalence to canonical sources.
...
- every registry pack includes generic export routing and alias compatibility, the shared summary and both exporters, English `.md` filenames, YAML requirements, artifact and fallback semantics, adaptive rendering, meaningful-closure rules, Notion compatibility, mastery/state separation, and no embedded credentials, opaque ids, local/vault paths, unsupported export claims, or active learner-state content;
