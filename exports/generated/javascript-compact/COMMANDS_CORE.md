# JavaScript Commands Core

Canonical sources: selected files under `commands/` and `specification/COMMAND_SPEC.md`.

## Supported Commands

- `START_LESSON`: start the next appropriate JavaScript lesson.
- `CONTINUE_LESSON`: continue an active lesson.
- `PRACTICE`: run output prediction, bug repair, code writing, refactoring, testing, async tracing, performance, security, or interview practice.
