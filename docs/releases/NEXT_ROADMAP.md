...
- Adds only `frontend-testing` as the tenth canonical domain for JavaScript/TypeScript and React engineers.
- Teaches testing economics, observable behavior, unit/component/integration/E2E boundaries, deterministic data and doubles, Vitest, React Testing Library, MSW, TanStack Query/router/auth integration, reliable Playwright architecture, CI, flaky-test diagnosis, coverage interpretation, legacy characterization tests, suite review, and strategy design.
- Requires explanation and mental models before guided and independent practice, realistic production cases, progressive hints, and learner-produced evidence rather than lesson completion or coverage percentage.
- Adds `frontend-testing-standard` with exactly 25 files and `frontend-testing-compact` with exactly five files, increasing generated packs from eighteen to twenty.
- Adds ADR-0059, launch kits, automated assertions, manual acceptance, and a planned pilot while preserving learner-facing metadata boundaries and generic Obsidian/Notion exports.
- Keeps Studio optional and generic; adds no testing-specific Studio screen, bridge action, browser runner, dependency, hosted service, learner runtime, release, or tag.
